...
**Changesets isn't yet integrated into this monorepo.** All packages currently have `private: true` (see `packages/*/package.json`) and version `0.0.0`. No `.changeset/` directory exists and no release automation runs. The `turbo.json`-based build pipeline handles compilation and testing, but package publishing isn't automated.

This makes sense for an internal template. The packages aren't published to npm. If any package in `apps/` or `packages/` is later extracted for independent consumption, add changesets then. <!-- vale fix: Microsoft.Contractions (L15:C57) + write-good.TooWordy (L15:C83) + write-good.Passive (L15:C199) -->
...
- **Pre-release (v3):** `@changesets/cli@3.0.0-next.x`, actively developed on `main` branch. The v3 rewrite has breaking changes and isn't yet stable. <!-- vale fix: Microsoft.Contractions (L31:C134) -->
- **For new integrations today:** stick with `^2.30.0` (the pnpm.io docs still reference v2.28.0 as the latest stable).

This repo doesn't currently have changesets installed.
...
| Concept | Description |
|---|---|
| **Changeset file** | A Markdown file in `.changeset/` with YAML front matter declaring packages, bump type, and a changelog summary. Created by the developer during PR review, not autogenerated. <!-- vale fix: Microsoft.Plurals (L42:C97) + Microsoft.Auto (L42:C185) --> |
| **Intent, not inference** | Unlike `semantic-release`, changesets doesn't parse commit messages. The bump type is always explicit and intentional. |
| **Version PR** | A GitHub Action opens a "Version Packages" PR that accumulates all pending changesets. It updates as new changesets land on `main`. This is the release gate. |
...
| **Internal dependency cascade** | When package A's version changes, all internal packages depending on A get a patch bump to update their `workspace:*` dependency range, governed by `updateInternalDependencies: "patch"` in config. <!-- vale fix: write-good.Passive (L47:C52) --> |
...
| **Changelog quality** | Human-written prose per-release | Autogenerated from commits |
| **Monorepo support** | First-class, independent versioning per package | Requires plugin (`semantic-release-monorepo`) |
| **PR review integration** | Explicit: the changeset is part of the PR | Implicit: semantic-release infers the version bump | <!-- vale fix: write-good.Passive (L145:C98) -->
...
## Best practices for commit messages and pull request titles
...
- CI lint tools (for example, `commitlint`) can enforce them independently.

**But don't rely on them for versioning.** A `fix:` prefix doesn't autobump; the changeset declares the actual semver impact. <!-- vale fix: Microsoft.Contractions (L162:C7) + Microsoft.Contractions (L162:C61) + Microsoft.Auto (L162:C70) -->
...
- The monorepo isn't a published library; it's an application deployment artifact. <!-- vale fix: Microsoft.Contractions (L210:C16) + Microsoft.Contractions (L210:C44) + write-good.TooWordy (L210:C44) -->
- Iteration happens fast and version history provides no value (internal tools).

**Current state of this repo:** All packages are `private: true`. Manual version management is currently unnecessary. Introduce Changesets only if/when a package goes public.

## Changesets and the issue to pull request workflow

This repo uses a structured workflow: `create-issue` → `triage` → `spec` → `implement` → `create-pr` → `review-pr`. Changesets slots into the `create-pr` and `review-pr` stages:
...
3. **During `review-pr`:** Reviewer sees the changeset alongside the code diff. The bot comment shows exactly what the release will produce.
...
`changeset version` runs and **consumes** the changeset file (deletes it). Re-applying it isn't possible. Merging a pull request with the wrong bump type forces manual recovery:

1. Manually correct the `package.json` version.
2. Correct the `CHANGELOG.md`.
3. Delete the consumed changeset file (already gone).
4. Another path is `changeset pre enter` / `changeset pre exit` for pre-release corrections.
...
- When running `pnpm changeset version`, always follow with `pnpm install` to update the lockfile after version bumps. Required before `pnpm publish -r`. <!-- vale fix: write-good.Passive (L267:C125) -->
...
- `changesets/action@v1` is compatible with turbo v2. No special integration applies.
- If using `turbo@2` with remote caching (`TURBO_TOKEN`/`TURBO_TEAM`), note that `changeset version` modifies files. Ensure the Version PR pipeline doesn't incorrectly serve cached artifacts for the `build` task. Use the `--force` flag in CI to bypass remote cache for build steps preceding publish. <!-- vale fix: Microsoft.Contractions (L274:C149) + write-good.Passive (L274:C241) -->

### Common Mistakes

1. **Forgetting to commit the changeset file.** The branch must carry the file in `.changeset/`. Without it, `changeset version` has nothing to process.
2. **Running `changeset version` without a follow-up `pnpm install`.** After version bumps, the lockfile is stale. Run `pnpm install` before publishing.
3. **Using the wrong bump type.** A patch is for bug fixes. A minor is for new backward-compatible functionality. A major is for breaking changes. When in doubt, ask: "Does this change require a consumer to update their code?" If yes, it's at least a minor.
...
If changesets landed in this monorepo, the config at `.changeset/config.json` would look like:
...
- `bumpVersionsWithWorkspaceProtocolOnly: true`: ensures changesets handles workspace protocol dependencies correctly with pnpm 11. <!-- vale fix: write-good.Passive (L311:C90) -->
