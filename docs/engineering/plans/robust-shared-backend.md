...
The same backend (`packages/api`, a Hono app) serves two distinct surfaces:
...
`apps/web` (marketing site) currently exposes only static pages (blog, changelog, legal). This plan adds two new routes: `/templates` and `/templates/[template_slug]`, both backed by the same `/api/v1/templates` endpoint that the CLI consumes. The "web" surface in this plan covers both `apps/app` (authenticated product) and `apps/web` (marketing, with the new templates pages).

The current architecture is functional but lacks the affordances of a multi-consumer API:

- The `Template` schema lives in two places (`packages/api/src/templates.ts` and `apps/cli/src/api.ts`), validated at runtime by a hand-written `isTemplate` type guard.
- The REST surface carries no version prefix (`/api/templates` instead of `/api/v1/templates`), stays unauthenticated by design, has no caching headers, no rate limiting, no pagination, no request IDs, and no global error handler beyond Hono's defaults.
...
- A comment in `packages/api/src/index.ts` labels `/templates` as "public for V1; gate behind `authGuard` in V1.1," but no coexistence path exists for the in-the-wild CLI clients.
- The API package's tests (`packages/api/tests/routes.test.ts`) construct local `new Hono()` instances with the same patterns; the real exported `api` isn't integration-tested, and no end-to-end test proves `apps/app`'s catch-all actually mounts it.

This plan is strategy. It doesn't write implementation code, open PRs, or touch production files.

## Goals

- Drop schema duplication between backend and CLI by introducing a single shared contract package.
- Version the REST surface so future breaking changes don't break installed CLI versions out from under users.
- Treat `/api/templates` as a real product: cacheable, paginated, filterable, rate-limited, documented.
- Make the CLI resilient to backend outages and to backend drift (CLI version behind server baseline).
...
- `/health` and `/ready` serve distinct goals: the former signals liveness, the latter pings Postgres and returns 503 on failure.
...
|---|---|---|
| Schema | Adding a required field on `TEMPLATE` breaks old CLI without warning (`isTemplate` rejects → `parse_error`) | No shared contract package; no address versioning |
...
- `apps/app/lib/orpc.ts` keeps its relative `RPCLink({ url: API_RPC_PATH })`. The client stays same-origin; cross-origin clients aren't in scope.
...
|---|---|---|---|---|
| 1 | New `@workspace/contracts` package with Zod schemas, plus `/api/v1` address prefix | Eliminates schema duplication; enables safe breaking changes later | Mechanical migration; one PR per consumer | **Now** |
