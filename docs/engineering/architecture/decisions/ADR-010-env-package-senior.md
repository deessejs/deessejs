...
│                       + the @next/env loader bridge. One file,
│                       one face. No fallback defaults inline.
│
├── client.ts           createEnv(clientSchema, undefined,
│                       { runtimeEnv, emptyStringAsUndefined: true })
│                       + non-fatal console.warn on parse failure.
│                       Fallback derives from
│                       clientSchema.parse({}), not from a literal.
│
└── loader.ts           the only file that talks to @next/env.
                        Captures loadEnvConfig result once; merges
                        into process.env; exposes loadRepoEnv().
                        Owns the #92040 workaround.
```
...
- **`schema.ts`** imports from `zod` only. It does not import
  from `@t3-oss/env-core`, `@next/env`, `node:process`, or any
...
- **`loader.ts`** imports `@next/env` and `node:path`. It does
  not import any validator. The validator cannot see the
  loader; the loader cannot see the validator; the schema is
  the meeting point.
