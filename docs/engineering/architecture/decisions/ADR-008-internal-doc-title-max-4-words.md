# ADR-008: internal-documentation page titles are four words or fewer
...
length budget, and is governed by the prose rules of the
body content, not this ADR.

A title that needs more than four words is **a signal**:
either the entry is conflating two concepts, or the entry
needs a section of its own, or the entry should be split
into two. The author restructures, not truncates.

## What this rule allows

- **Single-word titles** for pages that are genuinely
  one-word concepts (the section index pages:
  "Decisions", "Rules", "Knowledge Base").
...
- **The path** (folder + filename) is unaffected. A long
  path is acceptable; the title rendered is the
  frontmatter `title`, which is capped.

## What this rule forbids

- **Five-word or longer titles**. Rejected in review.
- **Truncation as a fix**: a title that reads as "Function
  Over Class for Public API" (cut from "Functions Over
  Classes for Public API") is rejected; the author
  splits the entry, or restructures, or renames the
  concept.
- **Acronyms** that hide length. "HonoEnv" is not a way
...
- **Re-introducing a section heading inside the body** that
  ...
  `## ...` heading, and is the right place for longer
  prose, not the title.
...
- **The meta.json `title`** field, which is the section
  ...
  budget is different. The tab title is the right length
  if it fits in one or two words; four words would be a
  different smell (too long for a tab).
- **The meta.json `description`** field, which is capped
  by an earlier convention at six words and a single list.
...
- **The folder names and file slugs** in
  ...
  can be longer than four words; the URL is read, not
  scanned.
...
The cap is checked at build time by the typecheck pass. A
...
- The `meta.json` schema, which carries its own length
  budget per field.
