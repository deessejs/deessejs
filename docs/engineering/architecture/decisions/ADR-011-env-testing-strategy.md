...
scripts, and the test runners. Today the package has zero tests. The
absence was masked by two things: (1) the earlier hand-rolled
...
2. **The alias regression** (PR #49 fix, 47789b9): a `z.preprocess`
   on individual fields cannot see sibling fields, so the resolution
   `AUTH_SECRET → BETTER_AUTH_SECRET` silently lost every alias. CI
...
no `process.env`. It is the only place that asserts the alias
...
correct value when either name is set in the snapshot, and pass the
...
guard is now one-shot at materialisation. A loop that injects a fake
key into the schema and asserts the throw covers the contract. The
guard's existence is documented in `packages/env/src/server.ts`;
...
the snapshot is a plain object that does not mutate `process.env` on
read.

The file is integration-tier because it depends on the host's
filesystem. It is also the only test that exercises `findRepoRoot` and
...
- **The `@workspace/vitest-config` shared preset** for every file.
  ...
  ADR-009's shared config; no per-package override is needed beyond
...
- **A coverage gate inside `packages/env/` separate from the
  monorepo-wide 80/75**. The package-level coverage is governed by
  ADR-009's shared `@workspace/vitest-config`. Adding a tighter
...
- **Coverage for `packages/env/` rises from 0 to a non-zero number
  ...
  roughly: `schema.ts` ~95% (every branch), `loader.ts` ~85% (the
  precedence branches are covered), `server.ts` ~70% (the materialise
  path is covered; the type-only `loadDotenvSnapshot` import side is
  not), `client.ts` ~60% (only the part that lives in the test
  surface).
...
- **The schema files** (`packages/env/src/schema.ts`). Tests assert
  the schema's behaviour; they do not edit it.
...
- **The `scripts/env-check.ts` validation script**. CI keeps
  running it; `packages/env/` tests do not duplicate that
  validation.
...
2. **A `tests/unit/csv.fuzz.test.ts`** using `zod-fast-check` to
   ...
   idempotence. Low-priority, low-cost, marginal gain.

Both are listed so the next iteration of this ADR can incorporate
them without re-justifying the choices.

## How to verify

After the test files are added:
...
4. The shared `@workspace/vitest-config` coverage threshold (80/75)
   is met for `packages/env/src/`. If not, the gate fails before
   merge.
...
- [ADR-002: File organization by sub-domain](./ADR-002-file-organization.md).
  The rule that test files are named after the concept they cover,
...
- [`@t3-oss/env-core` smoke test](https://github.com/t3-oss/t3-env/blob/main/packages/core/test/smoke-valibot.test.ts).
  ...
  `toThrow`). Future work in this ADR reproduces it for our
  `createEnv` surface.
