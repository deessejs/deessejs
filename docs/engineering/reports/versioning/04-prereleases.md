...
- If `@deessejs/cli` ever introduces a breaking-change opt-in (e.g. a new major version that requires user migration): use `pnpm changeset pre enter next`, publish pre-releases, then `pre exit` when stable.
