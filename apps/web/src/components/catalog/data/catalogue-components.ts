 ...
 * with V3 billing.
 *
 * V2 hand-maintained. V3 reads from
 * `packages/ui/src/components/*.tsx` (auto-generation) instead
 * of this list.
 */
...
export type ComponentTier = CatalogTier

export type ComponentCategoryId = CategoryId

// Generic parametrised on the slug and category unions. The
// shared `CatalogItem` keeps the type tight via the two type
// parameters.
export type CatalogueComponent = CatalogItem<
  string,
  ComponentCategoryId
>
...
export const CATALOGUE_COMPONENTS: CatalogueList<CatalogueComponent> = [
  // ── button (5) ───────────────────────────────────────────────
  {
    slug: "button",
    name: "Button",
