...
/**
 * PixelBlast — WebGL Bayer-dithered pixel background effect.
 *
 * Renders a fullscreen shader that produces a dithered FBM noise field.
 * Hovering with the pointer traces red trails that ripple outward as
 * decaying waves (when enableRipples is on). Designed for hero sections
 * where a subtle interactive background wants to anchor the page.
 *
 * Adapted from github.com/zavalit/bayer-dithering-webgl-demo.
 *
 * Requires `three` and `postprocessing` as runtime dependencies
 * (installed in apps/web).
 *
 * Must be rendered inside a relatively-positioned container with a
 * defined height — it fills its parent's box.
 */
