---
title: 'Env testing strategy'
description: 'Proposed (2026-08). Tests for packages/env.'
---
...
`packages/env/` ships five test files. Three are unit-tier and
load-bearing; two are integration-tier and surface the loader's
filesystem behaviour. The tests are organised so each file answers
one question, not so each tests one thing. Failures should localise
to a single file, and the file name should name what it covers, not
the schema it happens to exercise.
