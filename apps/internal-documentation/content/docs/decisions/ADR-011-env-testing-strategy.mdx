...
factors hid the absence: (1) the earlier hand-rolled
...
test suites. Both factors dissolved as the package evolved.
...
filesystem behaviour. The tests follow the same logic: each file answers
...
correct value when either name shows up in the snapshot, and pass the
...
guard's existence lives in `packages/env/src/server.ts`;
...
- **The `@workspace/vitest-config` shared preset** for every file.
  ...
  ADR-009's shared config; the package uses nothing past
  the test directory layout.
- **`vi.stubEnv` and `vi.unstubAllEnvs` for test isolation** when a
  test wants to set `process.env`. The shared config wires
  `unstubEnvs: true`, which autorestores between tests, the
  documented Vitest pattern for environment mutation.
- **A `@ts-expect-error` directive** in test files that exercise
  type-level guarantees (for instance, asserting that a server var carrying a
...
- **Mocking `node:fs`** in unit tests. The loader's filesystem
  contract surfaces in `tests/integration/loader.test.ts` against a
  real temp directory; mocking it elsewhere hides bugs at the loader
  boundary.
- **A custom error matcher for the `Validate env` script**. The
  script's output lives in the CI job, not in this package's
...
- **Coverage for `packages/env/` rises from 0 to a non-zero number
  ...
  precedence branches receive coverage), `server.ts` ~70% (the materialise
  path receives coverage; the type-only `loadDotenvSnapshot` import side
  stays untested), `client.ts` ~60% (only the part that lives in the test
  surface).
...
Both live here so the next iteration of this ADR can incorporate
...
2. `pnpm turbo run test` continues to show all packages green; the
   new env tests don't add regressions.
...
- [ADR-002: File organization by sub-domain](./ADR-002-file-organization.md).
  The rule that test files take their name from the concept they cover,
...
- [Rule 0006: Technology choices](../rules/0006-technology-choices.md).
  ...
  what it rules out. `vi.stubEnv` rules out manual process
