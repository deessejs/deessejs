...
for "see also" sections, "next steps" {/* vale fix: Vale.Spelling */}callouts, and
top-of-page entry points. They're also easy to misuse: a
...
contributor types. Code review on the doc source files enforces
the rule, not the site.

## Decision

When a Card renders in the rendering site, two rules
apply:

1. **Every Card has an icon**, when one is available.
   `"icon": "HomeIcon"` in the frontmatter, or an inline
   `<Icon>` element. A card without an icon leads to rejection
...
2. **The Card description is four lines or fewer**,
   ...
   exceeds four lines in the rendered grid leads to rejection in
...
description fails on both. The two rules stay independent:
...
either gets clipped (the contributor doesn't notice) or
overflows (the grid stops being uniform, the page breaks
visual rhythm). The contributor can't see this from the
...
visual hierarchy breaks; the reader doesn't know
...
default Fumadocs layout at the standard {/* vale fix: Vale.Spelling */}viewport. It isn't
...
- **A description just over four lines** when the
  contributor can show that the rendered output
  ...
  the line count.
- **A longer description in a Card that's a section
...
- **An icon that doesn't match the content.** A `BookIcon`
...
- The Cards component. Same as the Card component; the rule is "what you put
  inside a card," not "when you use a card grid."
- The frontmatter schema. The `icon` field is optional
  in the schema; the rule makes it required
  for the rendering site. The schema stays permissive
  because the source files in `docs/engineering/` aren't
  all rendering targets.

## How to enforce

Code review on the doc source files enforces the rule.
The reviewer checks the rendering of the affected
page in the dev server before approving the PR. The dev
server is `pnpm --filter @workspace/internal-documentation
dev`; the reviewer opens the page at the standard {/* vale fix: Vale.Spelling */}viewport
(1024px) and verifies each Card.

No automated check. The visual heuristic isn't amenable
to a Zod schema (a Card's appearance depends on the
{/* vale fix: Vale.Spelling */}viewport, the surrounding cards, and the icon used). The
...
- [ADR-004: API package structure](./ADR-004-api-package-structure.md),
  ...
  constraints get stated in the ADR, not enforced by tooling.
