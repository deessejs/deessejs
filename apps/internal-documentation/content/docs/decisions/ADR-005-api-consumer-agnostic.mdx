...
The contract describes itself from the server to the consumer; the
consumer describes itself in the consumer's own docs. The server
doesn't know who its clients are. It knows the contract; it
serves the contract; it doesn't name the consumer.
...
1. **A dedicated endpoint**: `packages/api/src/cli-version.ts`
   ...
   warn its user when the installed version falls below the
   server's floor. The endpoint's reason for being is the
   consumer.
2. **{/* vale fix: Vale.Spelling */}Docstrings that name the consumer**:
...
Each leak is small. Together they form a structural statement:
the API is the CLI's backend. The CLI is one consumer; the
API serves many. The naming and the {/* vale fix: Vale.Spelling */}docstrings tell the reader
...
The `packages/api/` package stays consumer-agnostic at every layer
that crosses the wire or the contract.

Concretely:

1. **No file in `packages/api/src/` takes its name from a consumer.**
   ...
   old name was a leak).
2. **No {/* vale fix: Vale.Spelling */}docstring in `packages/api/src/` names a consumer.**
   "Consumed by" is a view-from-the-consumer; the API's
   perspective is "the server exposes." {/* vale fix: Vale.Spelling */}Docstrings that say
   "the CLI does X" become "the server does X" or the team removes them.
3. **The exception: infrastructure that the consumer requires
   ...
   serves its version; the server doesn't serve "the CLI's
   version."
4. **The direction of consumer documentation reverses.**
   The mapping "CLI uses these endpoints" lives in
...
- **Tests that exercise the contract.** Server-side tests
  ...
  doesn't have to know who the real consumer is.
...
- **Variables, constants, files, or routes whose name embeds
  ...
  `appRouterFromCli`, `@cli/api`, and similar names, the server has no
  business naming things after a consumer.
- **{/* vale fix: Vale.Spelling */}Docstrings that describe the consumer.** "Consumed by,"
...
- **Imports that depend on a consumer's package.** The API
  ...
  internal core modules. The API doesn't import from
  `apps/cli`, `apps/web`, or `apps/app`. The reverse may
  happen (a consumer imports the API), but the server stays
  downstream of nothing in the apps folder.
- **Conditional logic based on who is calling.** The API
  doesn't look at a header, a user string, or a `referer` to
  decide "this is the CLI." The contract stays the same for
  every caller. If a caller needs different behaviour, the
  contract grows; the server doesn't branch on the
  caller's identity.
...
- `CLI_VERSION` → would the constant still describe the
  server's version if the CLI didn't exist? **Yes.** The
...
- `cli-version.ts` → would the file still describe the
  server's version endpoint if the CLI didn't exist?
...
- `cli-version` route → would the path still describe the
  ...
  the codebase can't coordinate with.
- "Consumed by apps/cli" in a {/* vale fix: Vale.Spelling */}docstring → would the
  statement still be meaningful? **No, the consumer is
  the source of the statement.** Remove the statement.

The test catches every leak this rule aims to prevent.

## Why this rule exists

The opposite of this rule is **"the API is the backend for
consumer X"**. This is the shape of a typical SaaS
application: the server gets built for the one client that
...
client's authors stay confused about which conventions apply
to them.
...
The consumer stays unremarkable to the server.
...
server that other consumers can't trust. A server that names
itself after the contract is a server that any consumer can
trust.

## Consequences

- A PR that introduces a file, constant, route, or {/* vale fix: Vale.Spelling */}docstring
  in `packages/api/` that names a consumer (`apps/cli`,
  `apps/web`, `apps/app`) leads to rejection. The author renames
  to the server's view.
- A PR that adds a server-side branch on caller identity
  (a header check, a user string check, a special token for
  one client) leads to rejection. The contract stays the same for
  every caller; richer contracts replace caller-identity
  branches.
- A PR that imports from `apps/*` inside `packages/api/`
  leads to rejection. The API stays downstream of nothing in the apps
  folder.
- The `clients.md` overview document that describes how
  each app consumes the API stays **in place**. It's the
  ...
  API; the rule doesn't forbid the mapping appearing in
  the consumers' docs.

## What this rule doesn't change

- The contract itself. The `appRouter` stays unchanged. The
  HTTP paths stay unchanged for the routes that don't
  contain a consumer's name (every route except
  `/cli-version`).
- The library imports inside `packages/api/`. The API
  keeps importing `@workspace/auth`, `@workspace/contracts`,
  `@workspace/env`, `@workspace/database`. These are
  dependencies, not consumers.
- The runtime topology. The catch-all in
  `apps/app/app/api/[[...route]]/route.ts` keeps mounting
  the API. The topology reverses later: the API
  is the engine, the apps host the routes, and the CLI is
  one caller.

## Where this rule came from

The rule came from `cli-version.ts`. The file existed
...
or any other consumer-named file the next contributor might
add.

The `"Consumed by: apps/cli"` {/* vale fix: Vale.Spelling */}docstring in
...
explains what the consumer does. The {/* vale fix: Vale.Spelling */}docstring is the wrong
...
3. Update the {/* vale fix: Vale.Spelling */}docstrings in `router/routes/templates.ts`,
...
- [ADR-002: File organization by sub-domain](./ADR-002-file-organization.md),
  the rule that files in a concern take their name from the
  concern. The API is its own concern; a consumer isn't
...
- [ADR-004: API package structure](./ADR-004-api-package-structure.md),
  ...
  `cli-version.ts`; the file lives in the right layer
