---
title: 'ADR-008, doc titles cap at four words'
...
length budget, and the prose rules of the
body content govern it, not this ADR.

A title that needs more than four words is **a signal**:
either the entry conflates two concepts, or the entry
needs a section of its own, or the author should split it
...
- **The path** (folder + filename) stays unaffected. A long
  path is acceptable; the title rendered is the
  frontmatter `title`, which the rule caps.

## What this rule forbids

- **Five-word or longer titles**. The reviewer rejects them.
- **Truncation as a fix**: a title that reads as "Function
  Over Class for Public API" (cut from "Functions Over
  Classes for Public API") leads to rejection; the author
...
- **The `meta.json` `title`** field, which is the section
  ...
  budget differs. The tab title is the right length
  if it fits in one or two words; four words would be a
  different smell (too long for a tab).
- **The `meta.json` `description`** field, which an earlier convention caps
  at six words and a single list.
...
- **The folder names and file slugs** in
  ...
  can run longer than four words; the URL gets read, not
  scanned.
...
architectural decision records. The cap is the longest entry that doesn't feel
...
- The metadata schema file, which carries its own length
  budget per field.
