...
description: 'Within a single concern (a feature, a module, a domain), code is'
---


## Rule

Within a single concern (a feature, a module, a domain), the codebase
separates code by **what it does**, not by **what kind of symbol
...
global. No `src/types.ts`, no `src/constants.ts` that holds
...
A type, a constant, and a function aren't interchangeable artefacts.
They live different lifecycles: a constant changes infrequently and reads
...
- **Syntax-based split**: `types/types.ts` holding every type from
  ...
  a lack of clarity on domain boundaries.
...
A helper that's genuinely cross-cutting (date formatting, string
...
- **Code review**. A reviewer who sees a `types.ts` at the package
  root that contains types from more than one concern blocks the PR.
...
- **Refactor signal**: when a `types.ts` or `constants.ts` starts
  mixing concerns, the team treats splitting it as a same-week
  cleanup, not a backlog item.

## Exceptions

None at the package root level. Within a single concern, a small
helper that lives next to its single caller may stay in the same
file (for example, an internal helper inside `validator.ts`); this
isn't a violation because the file names its operation, not
"helpers."

## See also

- **Rule 0003**, File Placement: the decision rule that picks the
  home for a file once the concern surfaces. This rule says
  "no cross-concern `types.ts`"; 0003 says "where does this new file
  go before writing it" and consolidates the codebase three
