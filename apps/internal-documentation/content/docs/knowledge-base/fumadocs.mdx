---
title: 'Fumadocs'
description: 'A study of the Fumadocs framework.'
---

# Fumadocs

A study of Fumadocs (the MDX-based documentation framework
used in the site's rendering app). Built on three upstream pages
about page conventions, the loader API, and collections.
...
Fumadocs isn't a single library. It's two pieces:
...
The two decouple cleanly: a custom UI can build on top of
`fumadocs-core` alone, and the renderer framework doesn't
...
`fumadocs-mdx` generates the `server.ts` file, but
`lib/source.ts` reads it at type-check time. If the
`postinstall` fails, the typecheck fails too, which is
the "no docs export" error a contributor sees when the
content directory is empty or the schema breaks.

## The loader , the runtime entry

The loader is the single point of access to the docs
content. `lib/source.ts` creates it:
...
`loader()` is **server-side only**. It isn't browser-safe
and isn't build-time-magic. It holds the parsed pages in
...
Slugs derive from the file path, not from the
frontmatter:
...
The page tree comes from reading the slugs and the
`meta.json` files. The `meta.json` in a folder controls:

- **Order**. The `pages` array overrides alphabetical.
- **Visibility**. The `pages` array forms the explicit
  allow list; items not listed get dropped.
...
|---|---|---|
| `h1` to `h6` | heading element props | Heading anchors (autogenerated TOC anchor) |
...
The `Card` props (the component requires `title`,
`description` and `icon` are optional, `href` makes it a
link, `external` marks
external links) and the `Callout` types
(`info | warn | error | success | warning | idea`) come from
the package's TypeScript definitions, not from the doc
site. The package itself sets the types.
...
The layout's MDX renderer wires in the default export.
