---
title: 'Commander.js'
description: 'A study of commander.js, the CLI framework.'
---

# Commander.js

A study of commander.js, the CLI framework. Built on
the upstream docs, this entry exists to show the shape of
the integration and the patterns a new command should follow.

## What commander offers
...
on whether a description accompanies it:
...
The first form fits cases where the child command loads
from a separate file (the parent only knows the name).
The second form fits cases where the child command defines
inline. The inline form works for a single-file
...
Handlers are async. The error a handler throws propagates
to the caller of `parseAsync`. The top-level catch serves
as the last-resort handler:
...
handler; the codebase keeps that for optional flags.
...
pattern applies to every boolean flag.

## What this entry doesn't use

- **No `exitOverride()`**, the CLI is a real CLI, not a
  library. `process.exit()` is the right behavior. Test code
  that needs to invoke the CLI programmatically adds
  `exitOverride()` then.
- **No variadic arguments**, every input is a flag.
- **No executable subcommands**. `program.command('list',
  'List available templates')` doesn't apply; the code base
  always chains. The two-call form is for cases where the
  child lives in a separate file (the code base doesn't
  need that yet).
- **No custom help formatting**, commander's default help
  works. `--help` wires automatically.
