...
No behavior change at runtime — the bundle was already inlining `@workspace/api`. The fix is to make the published `package.json` honest about what is and isn't a runtime install-time dependency, while keeping the package available to tsc and tsup through the dev-time workspace protocol.
