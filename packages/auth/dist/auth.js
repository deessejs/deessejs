 ...
 */
function logEmailFailure(flow, userId, error) {
    console.error(`[auth] ${flow} email failed`, JSON.stringify({ userId, flow, error }));
}
    ...
    emailAndPassword: {
        ...
        sendResetPassword: async ({ user, url }) => {
            void sendAuthEmail({
                ...
                if (!result.ok)
                    logEmailFailure("reset-password", user.id, result.error);
            });
        },
    },
    emailVerification: {
        ...
        sendVerificationEmail: async ({ user, url }) => {
            void sendAuthEmail({
                ...
                if (!result.ok)
                    logEmailFailure("verify-email", user.id, result.error);
            });
        },
    },
    session: {
