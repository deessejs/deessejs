 ...
 */
const orpcErrorBody = (code, status, message, data = {}, defined = false) => JSON.stringify({ defined, code, status, message, data });
/**
...
export const onError = (err, c) => {
    ...
    return new Response(orpcErrorBody("INTERNAL_SERVER_ERROR", 500, isProd ? "An unexpected error occurred" : err.message), {
        status: 500,
        headers: { "content-type": "application/json" },
    });
};
