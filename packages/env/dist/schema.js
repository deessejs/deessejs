...
export const canonicalUrl = z
    ...
    .url()
    .refine((v) => !v.endsWith("/"), {
    message: "Inter-app URL must not end with a trailing slash — " +
        "call sites use new URL(path, base) which already normalises.",
});
const csv = z.string().transform((s) => s
    .split(",")
    .map((p) => p.trim())
    .filter(Boolean));
/**
...
export const serverInputShape = {
    ...
    LOCALES: z
        ...
        message: "LOCALES must contain at least 2 locales (default + at least one non-default). A single-locale setup is not supported; see ADR-031 §1.",
    })
        .refine((s) => {
        const list = s
            .split(",")
        ...
        message: "LOCALES contains duplicates. Each locale must appear at most once.",
    }),
    DEFAULT_LOCALE: z
        .string()
        .min(1)
        ...
        message: "DEFAULT_LOCALE must be a BCP-47 language tag (e.g. 'en', 'fr', 'en-US').",
    }),
};
export const serverSchema = z.object(serverInputShape).superRefine((data, ctx) => {
    ...
    if (!data.DATABASE_URL) {
        ctx.addIssue({
            code: "custom",
            message: "DATABASE_URL is required in production (was optional in dev to keep onboarding simple)",
            path: ["DATABASE_URL"],
        });
    }
    ...
    if (!secret || secret.length < 32) {
        ctx.addIssue({
            code: "custom",
            message: "BETTER_AUTH_SECRET or AUTH_SECRET (>= 32 chars) is required in production. Run: openssl rand -base64 32",
    ...
    if (data.MAIL_TRANSPORT === "resend" && !data.RESEND_API_KEY) {
        ctx.addIssue({
            code: "custom",
            message: "RESEND_API_KEY is required when MAIL_TRANSPORT=resend (or use MAIL_TRANSPORT=console for dev)",
            path: ["RESEND_API_KEY"],
        });
    }
    if (!data.PARENT_DOMAIN) {
        ctx.addIssue({
            code: "custom",
            message: "PARENT_DOMAIN is required in production for cross-subdomain cookie sharing. Set to the bare hostname (e.g. deessejs.com).",
            path: ["PARENT_DOMAIN"],
        });
    }
...
export const clientSchema = z.object({
    ...
    NEXT_PUBLIC_DEFAULT_LOCALE: z
        ...
        .default("en")
        .refine((s) => /^[a-z]{2}(-[A-Z]{2})?$/.test(s), {
        message: "NEXT_PUBLIC_DEFAULT_LOCALE must be a BCP-47 language tag (e.g. 'en', 'fr').",
    }),
});
